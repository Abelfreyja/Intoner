...
internal sealed class SceneSelectionRenderer : IDisposable
{
    private const string ShaderResourceName = "Objects.Runtime.Selection.Shaders.SceneSelection.hlsl";

    private static readonly GpuShaderBytecode VertexShader = GpuShaderCompileService.CreateVertexShader(
            typeof(SceneSelectionRenderer),
            ShaderResourceName,
            "scene selection vertex shader");

    private static readonly GpuShaderBytecode PixelShader = GpuShaderCompileService.CreatePixelShader(
            typeof(SceneSelectionRenderer),
            ShaderResourceName,
            "scene selection pixel shader");
    ...
    public SceneSelectionRenderer(Device device, ISceneSelectionGeometryProvider geometryProvider)
    {
        _device = device;
        ...
        ConstantBuffer = new Buffer(
            device,
            Marshal.SizeOf<SelectionConstants>(),
        ...
        _meshCache = new SelectionMeshCache(device, geometryProvider);
    ...
    public void TouchSelectionPaths(SceneSelectionCollector collector, uint selectionId)
    {
        ...
        {
            _geometryProvider.Touch(modelPath);
    ...
    private void DrawCollector(SceneSelectionCollector collector, Matrix4x4 viewProjection)
    {
    ...
    private void DrawMesh(UploadedMesh mesh, uint selectionId, Matrix4x4 worldTransform, Matrix4x4 viewProjection)
    {
        ...
        var constants = new SelectionConstants
        {
        ...
        Context.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(mesh.VertexBuffer, Marshal.SizeOf<SelectionVertex>(), 0));
    ...
    private struct SelectionVertex
    {
        ...
        public SelectionVertex(Vector3 position)
        {
            Position = position;
        }
    }
    ...
    [StructLayout(LayoutKind.Sequential)]
    private struct SelectionConstants
    {
    ...
    private sealed class SelectionMeshCache : IDisposable
    {
        ...
        private readonly Dictionary<SceneSelectionPrimitiveKind, UploadedMesh> _primitiveMeshes = [];

        public SelectionMeshCache(Device device, ISceneSelectionGeometryProvider geometryProvider)
        {
            _device = device;
            _geometryProvider = geometryProvider;
        }
        ...
        public bool TryGetModelMesh(string modelPath, out UploadedMesh mesh)
        {
            ...
            {
                cachedEntry.LastTouchedUtc = DateTime.UtcNow;
                mesh = cachedEntry.Mesh;
                return true;
            }

            if (!_geometryProvider.TryGetGeometry(modelPath, out var geometry))
            {
                mesh = null!;
                return false;
            }
        ...
        public UploadedMesh GetPrimitiveMesh(SceneSelectionPrimitiveKind primitiveKind)
        {
            ...
            mesh = UploadGeometry(_device, SceneSelectionPrimitiveGeometry.Resolve(primitiveKind));
        ...
        private static UploadedMesh UploadGeometry(Device device, SceneSelectionGeometry geometry)
        {
            var vertices = new SelectionVertex[geometry.Positions.Length];
            for (var index = 0; index < geometry.Positions.Length; index++)
            {
                vertices[index] = new SelectionVertex(geometry.Positions[index]);
            }
