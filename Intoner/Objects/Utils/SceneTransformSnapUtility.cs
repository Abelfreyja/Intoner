...
internal static class SceneTransformSnapUtility
{
    public static Vector3 SnapPosition(Vector3 position, float step, in SceneSnapBasis basis)
        => SnapBasisPosition(position, step, basis, snapX: true, snapY: true, snapZ: true);

    public static Vector3 SnapPositionAxis(Vector3 position, int axisIndex, float step, in SceneSnapBasis basis)
    {
        if ((uint)axisIndex > 2)
        {
            return position;
    ...
    public static Vector3 SnapPositionAxes(Vector3 position, bool snapX, bool snapY, bool snapZ, float step, in SceneSnapBasis basis)
        => !snapX && !snapY && !snapZ
            ? position
    ...
    private static Vector3 SnapBasisPosition(Vector3 position, float step, in SceneSnapBasis basis, bool snapX, bool snapY, bool snapZ)
    {
    ...
    private static Vector3 ToBasisLocalPosition(Vector3 position, in SceneSnapBasis basis, out Quaternion rotation)
    {
        rotation = SceneTransformMath.NormalizeQuaternion(basis.Rotation);
