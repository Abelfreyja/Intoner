...
internal sealed partial class SceneItemInspector
{
    private bool TryResolveHousingPlacementEvaluation(ObjectSnapshot? snapshot, out PlacementEvaluation evaluation)
    {
        evaluation = default!;
        if (snapshot is null
            || !_sceneState.PlacementEvaluations.TryGetValue(snapshot.Id, out PlacementEvaluation? resolvedEvaluation)
    ...
    private static Vector4 ResolveHousingPlacementAccent(PlacementValidationStatus status)
        => status switch
        {
            PlacementValidationStatus.Valid   => ThemeColors.AccentGreen,
    ...
    private static Vector4 ResolveHousingPlacementMessageColor(PlacementEvaluation evaluation)
        => evaluation.Status == PlacementValidationStatus.Invalid
            ? ThemeColors.WithAlpha(ResolveHousingPlacementAccent(evaluation.Status), 0.95f)
    ...
    private static Vector4 ResolvePlacementFixAccent(PlacementFixKind kind)
        => kind switch
        {
            PlacementFixKind.SnapToSurface         => ThemeColors.AccentBlue,
            PlacementFixKind.MoveToPlayerPlacement => ThemeColors.AccentGreen,
            PlacementFixKind.ClearAttachmentParent => ThemeColors.AccentYellow,
